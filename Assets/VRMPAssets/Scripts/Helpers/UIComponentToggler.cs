...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            for (int i = 0; i < m_ComponentsToToggle.Count; i++)
            {
                if (m_ComponentsToToggle[i] != null)
                    m_ComponentsToToggle[i].enabled = show;
                else
                    Utils.Log($"Component Toggler on '{gameObject.name}': null entry at index {i} in ComponentsToToggle", 1);
            }

            for (int i = 0; i < m_ObjectsToToggle.Length; i++)
            {
                if (m_ObjectsToToggle[i] != null)
                    m_ObjectsToToggle[i].SetActive(show);
                else
                    Utils.Log($"Component Toggler on '{gameObject.name}': null entry at index {i} in ObjectsToToggle", 1);
            }
            ...
            if (m_DisableCanvasGroupObject)
            {
                if (m_CanvasGroup != null)
                    ...
                    Utils.Log($"Component Toggler on '{gameObject.name}': CanvasGroup is null", 1);
            }
